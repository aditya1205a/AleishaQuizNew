...
function displayQuestions(questions) {
    ...
    questions.forEach((q, index) => {
        if (!q || !q.question || !q.answer) return; // Skip invalid questions
...
function submitAnswers(e) {
    ...
    if (questions.length !== inputs.length) {
        showFeedback("😔 Oops! Something went wrong with the questions. Try again, Aleisha!");
        console.error("Question count mismatch:", questions.length, inputs.length);
        return;
    }
    const answers = [];
    let score = 0;
    inputs.forEach((input, index) => {
        if (!questions[index] || !questions[index].answer) {
            console.error(`Question ${index + 1} is undefined or missing answer`);
            return;
        }
    ...
    if (answers.length !== questions.length) {
        showFeedback("😔 Oops! Some questions couldn’t be processed. Try again, Aleisha!");
        return;
    }
